<div class="min-h-screen bg-zinc-50 dark:bg-zinc-900">
                ...
                <div class="flex items-center justify-between mb-4">
                    ...
                    @auth
                        @if(auth()->user()->id !== $post->user->id)
                            <button
                                wire:click="toggleFollow({{ $post->user->id }})"
                                class="px-3 py-1.5 text-xs font-semibold rounded-lg transition-colors duration-200 {{ $this->isFollowing($post->user->id) ? 'bg-zinc-200 dark:bg-zinc-700 text-zinc-800 dark:text-zinc-200 hover:bg-zinc-300 dark:hover:bg-zinc-600' : 'bg-blue-600 text-white hover:bg-blue-700' }}"
                            >
                        ...
                        @else
                            <div class="relative" x-data="{ open: false }">
                                ...
                                <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-2 w-48 bg-white dark:bg-zinc-800 rounded-md shadow-lg border border-zinc-200 dark:border-zinc-700 z-10">
                                    <div class="py-1">
                                        <flux:modal.trigger name="delete-post-modal">
                                            <button
    ...
    @if($showDeleteModal)
        <flux:modal name="delete-post-modal" class="min-w-[22rem]" :closable="false" :dismissible="false">
            <div class="space-y-6">
                <div>
